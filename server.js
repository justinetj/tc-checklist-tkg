...
const server = http.createServer(async (req, res) => {
  ...
  // ── Evite: standalone public event page with RSVP (guests never see the hub) ──
  const EVITE = {
    slug: "topgolf",
    title: "The Kumler Group x Zillow @ Top Golf",
    date: "Monday, Aug 17",
    time: "3:00pm – 5:00pm",
    venue: "Topgolf",
    address: "9500 E Talking Stick Way, Scottsdale, AZ",
    blurb: "Zillow is treating the team to an afternoon at Topgolf! Please RSVP so we can provide an accurate headcount. We hope you can join us!",
  };
  if (pathname === "/evite/" + EVITE.slug && req.method === "GET") {
    res.writeHead(200, { "Content-Type": "text/html" });
    return res.end(`<!DOCTYPE html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>${EVITE.title}</title>
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@600;800;900&display=swap" rel="stylesheet">
<style>
  * { box-sizing:border-box; margin:0; padding:0; }
  body { font-family:'Inter',sans-serif; min-height:100vh; color:white; padding:34px 20px 60px;
    background:linear-gradient(160deg,#7c1d0e 0%,#c2410c 35%,#ea580c 60%,#7c2d12 100%); }
  .wrap { max-width:560px; margin:0 auto; }
  h1 { font-size:clamp(34px,8vw,52px); font-weight:900; line-height:1.05; letter-spacing:-.02em; text-shadow:0 2px 18px rgba(0,0,0,.35); }
  .when { margin-top:20px; font-size:22px; font-weight:800; }
  .time { font-size:16px; font-weight:600; opacity:.9; margin-top:3px; }
  .where { margin-top:16px; font-size:15px; font-weight:600; }
  .where a { color:#ffd8b8; }
  .blurb { margin-top:18px; font-size:15px; line-height:1.55; font-weight:600; opacity:.95; }
  .btns { display:flex; gap:18px; margin:30px 0 8px; }
  .rsvp { flex:1; border:none; border-radius:20px; padding:22px 10px; font-family:inherit; font-weight:900; font-size:17px; cursor:pointer;
    background:rgba(255,255,255,.14); color:white; backdrop-filter:blur(6px); border:2px solid rgba(255,255,255,.35); transition:transform .1s; }
  .rsvp:hover { transform:scale(1.04); }
  .rsvp.on { background:white; color:#c2410c; }
  .rsvp .big { display:block; font-size:30px; margin-bottom:6px; }
  #form { display:none; margin-top:18px; background:rgba(255,255,255,.12); border:1px solid rgba(255,255,255,.3); border-radius:16px; padding:16px; backdrop-filter:blur(6px); }
  #form input { width:100%; border:none; border-radius:10px; padding:13px 14px; font-size:16px; font-family:inherit; font-weight:600; margin-bottom:10px; }
  #form button { width:100%; border:none; border-radius:10px; padding:14px; font-size:16px; font-family:inherit; font-weight:900; background:#16324f; color:white; cursor:pointer; }
  #done { display:none; margin-top:24px; font-size:22px; font-weight:900; text-align:center; background:rgba(255,255,255,.14); border-radius:16px; padding:26px 16px; }
</style></head><body><div class="wrap">
  <h1>${EVITE.title}</h1>
  <div class="when">${EVITE.date}</div>
  <div class="time">${EVITE.time}</div>
  <div class="where">📍 <strong>${EVITE.venue}</strong> · <a href="https://maps.apple.com/?q=${encodeURIComponent(EVITE.address)}">${EVITE.address}</a></div>
  <p class="blurb">${EVITE.blurb}</p>
  <div class="btns">
    <button class="rsvp" id="b-yes" onclick="pick(true)"><span class="big">✓</span>Going</button>
    <button class="rsvp" id="b-no" onclick="pick(false)"><span class="big">✕</span>Can't Go</button>
  </div>
  <div id="form">
    <input id="g-name" placeholder="Your name" autocomplete="name">
    <button onclick="send()">Send RSVP</button>
  </div>
  <div id="done"></div>
</div>
<script>
...
function pick(g) {
  going = g;
  document.getElementById('b-yes').classList.toggle('on', g);
  document.getElementById('b-no').classList.toggle('on', !g);
  document.getElementById('form').style.display = 'block';
  document.getElementById('g-name').focus();
}
async function send() {
  const name = document.getElementById('g-name').value.trim();
  if (!name) { alert('Add your name so we can count you!'); return; }
  await fetch('/api/evite/${EVITE.slug}/rsvp', { method:'POST', headers:{'Content-Type':'application/json'},
    body: JSON.stringify({ name, going }) });
  document.getElementById('form').style.display = 'none';
  document.querySelector('.btns').style.display = 'none';
  const d = document.getElementById('done');
  d.style.display = 'block';
  d.textContent = going ? "🎉 You're in, " + name + "! See you at Topgolf." : "💛 Thanks for letting us know, " + name + ". You'll be missed!";
}
</script></body></html>`);
  }
  if (pathname === "/api/evite/" + EVITE.slug + "/rsvp" && req.method === "POST") {
    let body = "";
    req.on("data", d => body += d);
    req.on("end", async () => {
      try {
        const { name, going } = JSON.parse(body || "{}");
        if (name && String(name).trim()) {
          await withData(data => {
            data.eviteRsvps = data.eviteRsvps || {};
            data.eviteRsvps[EVITE.slug] = data.eviteRsvps[EVITE.slug] || [];
            data.eviteRsvps[EVITE.slug].push({ name: String(name).trim().slice(0, 80), going: !!going, when: Date.now() });
          });
        }
        res.writeHead(200, { "Content-Type": "application/json" });
        res.end(JSON.stringify({ ok: true }));
  ...
  if (pathname === "/api/evite/" + EVITE.slug + "/rsvps.json" && req.method === "GET") {
    const tcq = url.searchParams.get("tc") || "";
    if (!(tcq === "admin" || ADMIN_TCS.includes(tcq))) { res.writeHead(403); return res.end("{}"); }
    const data = await loadData();
    res.writeHead(200, { "Content-Type": "application/json" });
    return res.end(JSON.stringify((data.eviteRsvps || {})[EVITE.slug] || []));
  }
  if (pathname === "/evite/" + EVITE.slug + "/list" && req.method === "GET") {
    const tcq = url.searchParams.get("tc") || "";
    if (!(tcq === "admin" || ADMIN_TCS.includes(tcq))) { res.writeHead(302, { Location: "/evite/" + EVITE.slug }); return res.end(); }
    const data = await loadData();
    const rs = ((data.eviteRsvps || {})[EVITE.slug] || []).slice().reverse();
    const yes = rs.filter(r => r.going), no = rs.filter(r => !r.going);
    res.writeHead(200, { "Content-Type": "text/html" });
    return res.end(`<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>RSVPs — ${EVITE.title}</title>
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@600;800&display=swap" rel="stylesheet">
<style>body{font-family:'Inter',sans-serif;background:#f2f5f9;color:#1c2733;padding:28px;max-width:560px;margin:0 auto}
h1{font-size:18px;font-weight:800;color:#16324f}.n{font-size:13px;color:#5b6b7f;font-weight:600;margin:4px 0 18px}
h2{font-size:13px;font-weight:800;text-transform:uppercase;letter-spacing:.05em;margin:18px 0 6px}
.g{color:#15803d}.b{color:#dc2626}div.r{background:white;border:1px solid #e5eaf1;border-radius:9px;padding:8px 13px;margin-bottom:6px;font-weight:600;font-size:14px}</style></head><body>
<h1>${EVITE.title}</h1><div class="n">${yes.length} going · ${no.length} can't go · ${rs.length} responses</div>
<h2 class="g">✓ Going (${yes.length})</h2>${yes.map(r => `<div class="r">${r.name.replace(/</g, "&lt;")}</div>`).join("") || '<div class="r" style="color:#8896a5">none yet</div>'}
<h2 class="b">✕ Can't go (${no.length})</h2>${no.map(r => `<div class="r">${r.name.replace(/</g, "&lt;")}</div>`).join("") || '<div class="r" style="color:#8896a5">none yet</div>'}
</body></html>`);
  }
